...
#Set my working directory as C:\Users\Arnav\Documents\R\win-library\3.4\DATA606\labs\Lab1\more #
...
How many cases are there in this data set: 20000 cases
How many variables: 9 variables
...
Excellent health proportion: 23%
...
Exercise 3: What does the mosaic plot reveal about smoking habits and gender? -The mosaic plot shows that men smoke more than 100 cigarettes.
...
wdiff = 0: correct weight.
wdiff > 0: need to add weight
wdiff < 0: need to lose weight.
...
Outcome: We can see from the plot that more people want to lose weight.
...
boxplot(wdiff_gender$wdiff ~ wdiff_gender$cdc.gender)
```

The above indicates that women are more interested in losing weight then men
...
Standard deviation is 40.080

Range for the 1 standard deviation:

```{r}
sd_above <- mean(cdc$weight)+sd(cdc$weight) 
```

```{r}
sd_below <- mean(cdc$weight)-sd(cdc$weight) 
```

sd_above = 209.76
sd_below = 129.60

Now we have to see how many records are between sd_below and sd_above.


```{r}
one_sd <- sum(cdc$weight> 129.60 & cdc$weight < 209.76)
```

so total number if people with in one standard deviation = 14152

Proportion of the weights are within one standard deviation

```{r}
Percentage_one_sd <- (14152/20000)*100
```

Proportion of the weights are within one standard deviation = 70.76
